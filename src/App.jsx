...
export default function HomeBarInventory() {
  ...
  useEffect(() => {
    ...
    const inventoryPath = typeof __app_id !== 'undefined' 
        ? collection(db, 'users', user.uid, 'inventory') 
        : collection(db, 'users', user.uid, 'inventory');
        
    const q = query(inventoryPath);
    ...
    const settingsPath = typeof __app_id !== 'undefined'
        ? doc(db, 'users', user.uid, 'settings', 'preferences')
        : doc(db, 'users', user.uid, 'settings', 'preferences');

    const unsubSettings = onSnapshot(settingsPath, (docSnap) => {
      if (docSnap.exists()) {
  ...
  const saveBarName = async () => { if(user) await updateDoc(getDocRef('settings'), { barName }).catch(()=>{}); };
  const addCategory = async () => { if(!newCatName.trim()) return; const updated = [...categories, newCatName.trim()]; setCategories(updated); setNewCatName(''); if(user) await updateDoc(getDocRef('settings'), { categories: updated }).catch(()=>{}); };
      ...
      <main className="max-w-5xl mx-auto px-4 pt-6">
        ...
        {!activeCategory && !selectionMode && (
          <div className="grid grid-cols-2 gap-4 mb-3">
             ...
             <div onClick={()=>setActiveCategory('SPIRITS')} className={`relative overflow-hidden rounded-2xl p-5 border cursor-pointer ${theme.dashboardMainBg} hover:opacity-90`}><p className={`text-xs font-bold uppercase ${theme.dashboardMain}`}>Total Bottles</p><p className="text-3xl font-bold mt-1 text-white">{spiritsCount}</p><BottleIcon className={`absolute -right-2 -bottom-2 opacity-10 text-white`} size={80} /></div>
          </div>
        )}
